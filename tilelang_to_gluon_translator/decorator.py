...
from .version_check import get_gluon_version
...
class TileLangGluonWrapper:
    ...
    def __init__(
        ...
        self._base_source_code = self.source_code
    ...
    def _is_gemm_kernel(self) -> bool:
        """Return whether the extracted TileLang source contains GEMM ops."""
        return "T.gemm(" in self.source_code
    ...
    def _should_retry_with_pointer_mode(self, exc: Exception) -> bool:
        """Retry GEMM kernels in pointer mode for Triton/Gluon 3.6+ known failures."""
        if self.translator.use_pointer_mode or self._auto_pointer_retry_attempted:
            return False
        if not self._is_gemm_kernel():
            return False
        if self._has_atomic_add():
            return False
        if self._installed_gluon_version() < (3, 6):
            return False

        text = f"{type(exc).__name__}: {exc}"
        known_failures = [
            "accumulator shape is incompatible",
            "expected 'layout' to be a SharedLayout but got NVMMADistributedLayout",
            "shared_memory_descriptor",
        ]
        return any(snippet in text for snippet in known_failures)

    def _retry_compile_with_pointer_mode(self) -> None:
        """Switch translator to pointer mode and invalidate current compiled state."""
        self._auto_pointer_retry_attempted = True
        self.source_code = "# pointer_mode_retry\n" + self._base_source_code
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            if not self._should_retry_with_pointer_mode(exc):
                raise
            self._retry_compile_with_pointer_mode()
            self._compiled_kernel = self._compile_kernel()
            return self._compiled_kernel(*torch_args, **kwargs)
