...
class GluonCodeGenerator:
    ...
    def _generate_kernel(self, kernel: GluonKernel):
        ...
        for reg in getattr(kernel, 'register_tensors', []):
            layout_expr = reg.layout if getattr(reg, "layout", None) else "gl.BlockedLayout([1, 1], [1, 32], [4, 1], [1, 0])"
            constexpr_params.append(f"{reg.name}_layout: gl.constexpr = {layout_expr}")
    ...
    def _generate_register_tensor(self, stmt: GluonRegisterTensor, in_dtype: str = "gl.float32", out_dtype: str = "gl.float32"):
        ...
        self.lines.append(
            f"{self._indent()}{stmt.name} = gl.full("
            f"{shape_str}, 0, {actual_dtype}, layout={layout_name})"
        )
